 ...
 * @returns string: base64 encoded image
 */
export async function getWholeQR(orgId: string, shortId: string, longId: string): Promise<SomeResult<Buffer>> {

  const qrCodeResult = await generateQRCode(orgId, longId);
  if (qrCodeResult.type === ResultType.ERROR) {
    return qrCodeResult;
  }
  ...
  return new Promise((resolve, reject) => {
    ...
    svg2img(svgString, function (error, buffer: Buffer) {
      if (error) {
        resolve(makeError(error));
        return;
      }
      
      resolve(makeSuccess(buffer));
    });
  });
}