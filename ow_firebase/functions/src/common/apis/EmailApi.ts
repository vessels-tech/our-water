...
import {
  ...
  makeSuccess,
  makeError
} from "ow_common/lib/utils/AppProviderTypes";
import {
...
export default class EmailApi {
  ...
  public static async sendUserDigestEmail(
    email,
    users: Array<User>
    ...
    if (!shouldSendEmails && testEmailWhitelist.indexOf(email) === -1) {
      console.log(
        `Not sending emails as shouldSendEmails is false, and ${email} is not in the whitelist.`
      );
  ...
  public static sendResourceEmail(
    email,
    subject,
    message,
    attachments
