...
var Provider = function (_React$Component2) {
  ...
  function Provider(props) {
    ...
    _this2.updateStore = function (updatedStore, callback) {
      _this2.setState(_extends({}, _this2.state, updatedStore), async function () {
        await _this2.persist();
    ...
    _this2.componentWillMount = async function () {

      if (_this2.props.persist !== false) {
        console.log("getting saved store!");
        var savedStore = await _this2.props.persist.storage.getItem(_this2.props.persist.key || 'react-context-api-store');
        console.log("got saved store", savedStore);

        //TODO: make async!
        // var savedStore = '{"isConnected":true}';

        _this2.setState({ ..._extends({}, _this2.props.store, _this2.props.persist.statesToPersist(JSON.parse(savedStore) || {})) });
