...
export default class MyWellApi implements BaseApi, UserApi, InternalAccountApi {
  ...
  async saveReading(
  ...
  ): Promise<SomeResult<SaveReadingResult>> {
    ...
    const saveResult = await FirebaseApi.saveReadingPossiblyOffineToUser(
      this.orgId,
      userId,
      reading
    );
  ...
  async addRecentResource(
    ...
    //Remove the resource
    const userApi = new CommonUserApi(<any>fs, this.orgId);
    const newResourceResponse = await userApi.removeNewResource(
      userId,
      resource.id
    );
  ...
  addFavouriteResource(
    ...
    return FirebaseApi.addFavouriteResource(this.orgId, resource, userId).then(
      () => {
        const result: SomeResult<void> = {
  ...
  removeFavouriteResource(
    ...
    return FirebaseApi.removeFavouriteResource(
      this.orgId,
      resourceId,
      userId
    ).then(() => {
  ...
  async getResourcesWithinRegion(
    region: Region
  ): Promise<SomeResult<AnyResource[]>> {
    //TODO: for all of the resources, also get the short Id before returning
    const getResourcesResult = await FirebaseApi.getResourcesWithinRegion(
      this.orgId,
      region
    );
   ...
   */
  async getResourcesWithinRegionPaginated(
    region: Region,
    cursor: Cursor
  ...
  ): Promise<SomeResult<SaveResourceResult>> {
  ...
  deletePendingResource(
    ...
    FirebaseApi.deletePendingReadingsForResource(
      this.orgId,
      userId,
      pendingResourceId
    );
  ...
  async getShortId(resourceId: string): Promise<SomeResult<string>> {
    ...
    const getShortIdResult = await FirebaseApi.getShortId(
      this.orgId,
      resourceId
    );
  ...
  async preloadShortIds(ids: string[]): Promise<SomeResult<string[]>> {
    ...
    const getShortIdResults = await Promise.all(
      ids.map(id => this.getShortId(id))
    );
    ...
    const shortIds: string[] = getShortIdResults.map(
      r => r.type === ResultType.SUCCESS && r.result
    );
   ...
   * @param userId
   */
  async runInternalSync(
    userId: string
  ): Promise<SomeResult<ExternalSyncStatusComplete>> {
    ...
    return (
      ftch(syncUrl, options)
        ...
        .catch((err: Error) =>
          makeError<ExternalSyncStatusComplete>(err.message + err.stack)
        )
    );
  }
   ...
   */
  async getReadingsForTimeseries(
    resourceId: string,
    timeseriesName: string,
    timeseriesId: string,
    range: TimeseriesRange
  ): Promise<AnyReading[]> {
    const result = await FirebaseApi.getReadings(
      this.orgId,
      resourceId,
      timeseriesName,
      range
  ...
  subscribeToPendingReadings(
    userId: string,
    callback: (resources: PendingReading[]) => void
  ): () => void {
    return FirebaseApi.listenForPendingReadingsToUser(
      this.orgId,
      userId,
      callback
    );
  ...
  subscribeToPendingResources(
    userId: string,
    callback: (resources: PendingResource[]) => void
  ): () => void {
    return FirebaseApi.listenForPendingResourcesToUser(
      this.orgId,
      userId,
      callback
    );
  ...
  subscribeToUser(userId: string, callback: any): () => void {
    return FirebaseApi.listenForUpdatedUser(
      this.orgId,
      userId,
      (sn: Snapshot) => callback(sn)
    );
  }
  ...
  sendVerifyCode(
    mobile: string
  ): Promise<SomeResult<RNFirebase.ConfirmationResult>> {
    return FirebaseUserApi.sendVerifyCode(mobile);
  }

  verifyCodeAndLogin(
    confirmResult: RNFirebase.ConfirmationResult,
    code: string,
    oldUserId: string
  ): Promise<SomeResult<FullUser>> {
    return FirebaseUserApi.verifyCodeAndLogin(
      this.orgId,
      confirmResult,
      code,
      oldUserId
